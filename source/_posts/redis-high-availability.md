---
title: Redis高可用是如何做的？浅析哨兵的工作原理
date: 2020-06-29 10:36:19
categories: [Redis]
tags: [redis, 高可用]
---

一个优秀的软件，除了要保证高性能，还要有完善的高可用机制，保障软件的稳定性。

Redis作为非常火热的内存数据库，其除了具有非常高的性能之外，在高可用方面也有完善的支持。这篇文章我们就来分析一下，Redis是如何实现高可用的，以及我们经常听到的哨兵是什么，它的工作原理是怎样的。
...
从上面几种部署模式可以看出，提高Redis可用性的思路是：**多副本部署 + 自动故障恢复**。
